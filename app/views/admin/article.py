...
## 详情
@admin.route('/article/view/<int:id>')
def article_view(id):
    meta = {
        'title': '文章管理',
...
def article_submit():
    ...
    return render_template('admin/article/submit.html', meta=meta, form=form)

## 保存
@admin.route('/article/save', methods=['POST'])
def article_save():
    meta = {
        'title': '文章管理',
        'css_nav_sub_product': 'active',
    ...
    if form.validate_on_submit():
        ...
        if product:
            return jsonify(success=True, message='操作成功!', redirect_to=url_for('admin.article_list'))
        else:
...
def article_delete():
    ...
    return jsonify(success=True, message='操作成功!', data={'ids': ids, 'type':type}, redirect_to=url_for('admin.article_list'))
